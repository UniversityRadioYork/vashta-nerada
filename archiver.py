...
def archive(directory: str, archive_location: str, ttl: int, mode: str, dry_run: bool) -> None:
    ...
    if mode == "unit":
        ...
        for subdirectory in subdirectories:
            _mtime = datetime.datetime.fromtimestamp(os.stat(os.path.join(directory, subdirectory)).st_mtime)
            ...
            with tarfile.open(f"{config.ARCHIVE_LOC}/{fn}.tar.gz", "w:gz") as tar:
                tar.add(os.path.join(directory, subdirectory), arcname=subdirectory)

            with open(f"{config.LIBRARY_LOC}/{fn}.fofn", "w") as fofn:
                fofn.write("\n".join(all_entries(os.path.join(directory, subdirectory))))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dry", help="run archiver without deleting files once archived", action="store_true")
